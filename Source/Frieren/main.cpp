...
int main()
{
    CoFileWatcher watcher{ "Assets"_fs, EFileWatchFilterFlags::ChangeSize | EFileWatchFilterFlags::ChangeFileName, true };
    while (true)
    {
        std::vector<FileChangeInfo> infos = watcher.RequestChanges(true, false);
        for (const FileChangeInfo& info : infos)
        {
            IG_LOG(LogTemp, Debug, "{}, {}", magic_enum::enum_name(info.Action), info.Path.string());
        }
    }
