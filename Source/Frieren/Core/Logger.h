...
namespace fe
{
	...
	public:
		...
		{
			spdlog::logger& logger = RegisterCategory<C>();
	...
	private:
		template <typename C>
		spdlog::logger& RegisterCategory()
		{
			if (!categoryMap.contains(HashOfType<C>))
			{
				categoryMap[HashOfType<C>] = new spdlog::logger(C::CategoryName.data(), { consoleSink, fileSink });
				categoryMap[HashOfType<C>]->set_level(spdlog::level::trace);
			}

			return *categoryMap[HashOfType<C>];
		}
