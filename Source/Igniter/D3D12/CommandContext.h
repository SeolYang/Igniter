...
namespace ig
{
    ...
    public:
        ...
        void AddPendingBufferBarrier(GpuBuffer& targetBuffer,
                                     ...
                                     const size_t sizeAsBytes = std::numeric_limits<size_t>::max());
        void FlushBarriers();
        ...
        void CopyBuffer(GpuBuffer& src, const size_t srcOffsetInBytes, const size_t numBytes, GpuBuffer& dst,
                        const size_t dstOffsetInBytes);
        void CopyTextureRegion(GpuBuffer&     src, const size_t srcOffsetInBytes, GpuTexture& dst,
                               const uint32_t subresourceIdx, const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& layout);
        ...
        void SetViewport(const float topLeftX, const float       topLeftY, const float width, const float height,
        ...
        void SetRoot32BitConstants(const uint32_t registerSlot, const uint32_t num32BitValuesToSet, const void* srcData,
        ...
        void SetRoot32BitConstants(const uint32_t registerSlot, const T& data, const uint32_t destOffsetIn32BitValues)
        {
            ...
            SetRoot32BitConstants(registerSlot, sizeof(T) / 4, reinterpret_cast<const void*>(&data),
