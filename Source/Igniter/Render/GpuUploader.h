...
namespace ig
{
    ...
    public:
        ...
        bool IsValid() const
        {
            return (uploadBuffer != nullptr) && (offsettedCpuAddr != nullptr) && (request != nullptr);
        }
        ...
        void WriteData(const uint8_t* srcAddr, const size_t srcOffsetInBytes, const size_t destOffsetInBytes,
                       const size_t   writeSizeInBytes);

        void CopyBuffer(const size_t srcOffsetInBytes, const size_t numBytes, GpuBuffer& dst,
                        const size_t dstOffsetInBytes = 0);
        void CopyTextureRegion(const size_t srcOffsetInBytes, GpuTexture& dst, const uint32_t subresourceIdx,
                               const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& layout);
        void CopyTextureSimple(GpuTexture& dst, const GpuCopyableFootprints& dstCopyableFootprints,
