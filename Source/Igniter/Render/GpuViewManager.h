...
namespace ig
{
    ...
    public:
        ...
        Handle<GpuView, GpuViewManager*> RequestConstantBufferView(GpuBuffer&     gpuBuffer, const uint64_t offset,
        ...
        Handle<GpuView, GpuViewManager*> RequestShaderResourceView(GpuTexture&              gpuTexture,
                                                                   const GpuTextureSrvDesc& srvDesc,
                                                                   const DXGI_FORMAT        desireViewFormat =
                                                                           DXGI_FORMAT_UNKNOWN);
        Handle<GpuView, GpuViewManager*> RequestUnorderedAccessView(GpuTexture&              gpuTexture,
                                                                    const GpuTextureUavDesc& uavDesc,
                                                                    const DXGI_FORMAT        desireViewFormat =
                                                                            DXGI_FORMAT_UNKNOWN);
        Handle<GpuView, GpuViewManager*> RequestRenderTargetView(GpuTexture&              gpuTexture,
                                                                 const GpuTextureRtvDesc& rtvDesc,
                                                                 const DXGI_FORMAT        desireViewFormat =
                                                                         DXGI_FORMAT_UNKNOWN);
        Handle<GpuView, GpuViewManager*> RequestDepthStencilView(GpuTexture&              gpuTexture,
                                                                 const GpuTextureDsvDesc& dsvDesc,
                                                                 const DXGI_FORMAT        desireViewFormat =
                                                                         DXGI_FORMAT_UNKNOWN);
