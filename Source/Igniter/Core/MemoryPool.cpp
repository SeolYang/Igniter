...
namespace ig
{
    MemoryPool::MemoryPool() : sizeOfElement(0), alignOfElement(0), numInitialElementPerChunk(0), magicNumber(InvalidMagicNumber) {}

    MemoryPool::MemoryPool(const size_t newSizeOfElement, const size_t newAlignOfElement, const uint16_t numElementPerChunk,
        const uint32_t numInitialChunk, const uint16_t magicNumber)
    ...
    {
        IG_CHECK(sizeOfElement >= sizeof(uint64_t) &&
    ...
    bool MemoryPool::CreateNewChunk()
    {
        if (chunks.size() + 1 != MaxNumChunk)
        {
            ...
            chunks.emplace_back(
                static_cast<uint8_t*>(_aligned_malloc(sizeOfElement * numInitialElementPerChunk, std::hardware_destructive_interference_size)));
