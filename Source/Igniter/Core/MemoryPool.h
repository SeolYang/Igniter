...
namespace ig
{
    ...
    public:
        ...
        uint8_t* GetAliveAddressOf(const uint64_t handle) { return IsAlive(handle) ? GetAddressOf(handle) : nullptr; }

        const uint8_t* GetAliveAddressOf(const uint64_t handle) const { return IsAlive(handle) ? GetAddressOf(handle) : nullptr; }

        bool IsValid(const uint64_t handle) const
        {
            return handle != InvalidHandle && ((MaskMagicNumber(handle) == magicNumber) && (MaskChunkIndex(handle) < chunks.size()) &&
    ...
    private:
        ...
        {
            return static_cast<uint64_t>(magicNumber) << MagicNumberOffset | static_cast<uint64_t>(chunkIdx) << ChunkIndexOffset |
